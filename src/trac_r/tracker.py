...
class ExperimentTracker:
    def __init__(self, run_name: str, root_dir: str = "tracker", hf_token: str = None):
        ...
        existing_runs = [d for d in os.listdir(self.root_dir) if os.path.isdir(os.path.join(self.root_dir, d))]
    ...
    def register_chart(self, title: str, filename: str, chart_type: str, options: dict = None):
    ...
    def sync_to_hf(self, repo_id: str, cleanup: bool = False):
        ...
        if not token:
            ...
            token = getpass.getpass("Paste your HF Read/Write Token here (input is hidden): ")
    ...
    def save_trajectory(self, true_data, pred_data, filename="trajectory.csv"):
        ...
        with open(csv_path, 'w', newline='') as f:
            writer = csv.writer(f)
            writer.writerow(['step', 'true_dim0', 'true_dim1', 'pred_dim0', 'pred_dim1'])
    ...
    def save_confusion_matrix(self, matrix_data, classes=None, title="Confusion Matrix", filename="confusion.csv"):
        """
        ...
        with open(csv_path, 'w', newline='') as f:
            writer = csv.writer(f)
            writer.writerow(['true_class', 'pred_class', 'count', 'true_label', 'pred_label'])
