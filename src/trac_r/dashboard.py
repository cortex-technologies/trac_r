                ...
                try:
                    ...
                    for run_name in sorted(run_folders, reverse=True):
                        ...
                        if f"{run_name}/meta.json" in repo_files:
                            try:
                                local_meta_path = hf_hub_download(repo_id=REPO_ID, repo_type="dataset", filename=f"{run_name}/meta.json")
            ...
            if REPO_ID and HfApi and parsed_path.path.startswith("/run"):
                ...
                try:
                    ...
                    with open(local_path, 'rb') as f:
                        content = f.read()
                    self.send_response(200)
                    self.send_header('Content-type', 'text/csv' if filename.endswith('.csv') else 'application/json')
                    self.end_headers()
...
def main():
    ...
    parser.add_argument("--repo-id", type=str, default=os.environ.get("HF_REPO_ID"), help="Hugging Face Dataset Repo ID (e.g. username/dataset)")
